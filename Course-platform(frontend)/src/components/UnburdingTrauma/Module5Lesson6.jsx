import React from 'react'

const Module5Lesson6 = () => {
  return (
    <div className="p-8">
      <h1 className="text-2xl font-bold text-[#bd6334] mb-6">Breathwork for Emotional Clearing</h1>
      
      <div className="prose max-w-none">
        <p className="text-lg leading-relaxed mb-6">
          Welcome to this transformative lesson on breathwork for emotional clearing. Today, we explore how conscious breathing can become your most accessible and powerful tool for releasing stored emotions and healing trauma patterns.
        </p>

        <h2 className="text-2xl font-semibold text-[#bd6334] mt-8 mb-4">The Power of Conscious Breathing</h2>
        <p className="text-lg leading-relaxed mb-6">
          Your breath is one of the most powerful tools available for emotional release and clearing. Conscious breathing practices create direct access to your autonomic nervous system, allowing you to release stored tension patterns and facilitate the movement of stuck emotional energy.
        </p>

        <div className="bg-blue-50 p-6 rounded-lg mb-6">
          <h3 className="text-xl font-semibold text-[#bd6334] mb-3">Why Breathwork Works for Emotional Release:</h3>
          <ul className="list-disc list-inside space-y-2">
            <li>Activates the parasympathetic nervous system for deeper relaxation</li>
            <li>Increases energy flow to areas of physical/emotional holding</li>
            <li>Creates a safe container for emotional release without needing to narrate</li>
            <li>Reduces the physical burden of emotional suppression</li>
            <li>Works directly with your physiology to reverse restrictive patterns</li>
            <li>Provides immediate regulation when emotions feel overwhelming</li>
          </ul>
        </div>

        <h2 className="text-2xl font-semibold text-[#bd6334] mt-8 mb-4">Understanding the Breath-Emotion Connection</h2>
        <p className="text-lg leading-relaxed mb-6">
          When emotions are suppressed, our breathing patterns often become restricted as part of containing those feelings. By changing how we breathe, we can reverse this process and create space for emotional energy to move and release.
        </p>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          <div className="bg-yellow-50 p-6 rounded-lg">
            <h3 className="text-xl font-semibold text-[#bd6334] mb-3">How Emotions Affect Breathing</h3>
            <ul className="list-disc list-inside space-y-2">
              <li><strong>Anxiety:</strong> Shallow, rapid breathing in the chest</li>
              <li><strong>Anger:</strong> Forceful exhales, clenched breath</li>
              <li><strong>Sadness:</strong> Irregular patterns, holding breath</li>
              <li><strong>Fear:</strong> Breath held high in chest, hyperventilation</li>
              <li><strong>Depression:</strong> Shallow, minimal breathing</li>
            </ul>
          </div>
          
          <div className="bg-green-50 p-6 rounded-lg">
            <h3 className="text-xl font-semibold text-[#bd6334] mb-3">How Conscious Breathing Helps</h3>
            <ul className="list-disc list-inside space-y-2">
              <li>Deepens breathing to activate calm response</li>
              <li>Releases physical tension patterns</li>
              <li>Moves stagnant emotional energy</li>
              <li>Creates space between feeling and reaction</li>
              <li>Restores natural breathing rhythms</li>
            </ul>
          </div>
        </div>

        <h2 className="text-2xl font-semibold text-[#bd6334] mt-8 mb-4">Breathwork Techniques for Emotional Clearing</h2>
        <p className="text-lg leading-relaxed mb-6">
          These techniques are specifically designed to help you process and release stored emotions safely:
        </p>

        <div className="space-y-6 mb-8">
          <div className="bg-purple-50 p-6 rounded-lg">
            <h3 className="text-xl font-semibold text-[#bd6334] mb-3">1. Three-Part Breath (Dirga Pranayama)</h3>
            <p className="mb-3">A gentle foundation practice that promotes deep relaxation and emotional regulation.</p>
            
            <h4 className="font-semibold mb-2">Instructions:</h4>
            <ol className="list-decimal list-inside space-y-1 mb-3">
              <li>Lie down comfortably with one hand on chest, one on belly</li>
              <li>Inhale slowly into your belly (hand rises)</li>
              <li>Continue inhaling into your ribcage (ribs expand sideways)</li>
              <li>Complete the breath into your upper chest (chest hand rises)</li>
              <li>Exhale in reverse order: chest, ribs, belly</li>
              <li>Continue for 5-10 minutes</li>
            </ol>
            
            <p className="text-sm italic">Best for: Anxiety, overwhelm, establishing breath awareness, daily regulation</p>
          </div>

          <div className="bg-orange-50 p-6 rounded-lg">
            <h3 className="text-xl font-semibold text-[#bd6334] mb-3">2. 4-7-8 Breathing for Emotional Regulation</h3>
            <p className="mb-3">Activates the parasympathetic nervous system to calm intense emotions.</p>
            
            <h4 className="font-semibold mb-2">Instructions:</h4>
            <ol className="list-decimal list-inside space-y-1 mb-3">
              <li>Exhale completely through your mouth</li>
              <li>Close your mouth, inhale through nose for 4 counts</li>
              <li>Hold your breath for 7 counts</li>
              <li>Exhale through mouth for 8 counts (making a "whoosh" sound)</li>
              <li>Repeat cycle 4 times initially, building to 8 cycles</li>
            </ol>
            
            <p className="text-sm italic">Best for: Panic, anger, insomnia, acute stress, emotional overwhelm</p>
          </div>

          <div className="bg-blue-50 p-6 rounded-lg">
            <h3 className="text-xl font-semibold text-[#bd6334] mb-3">3. Cleansing Breath (Kapalabhati)</h3>
            <p className="mb-3">An energizing practice that helps move stagnant emotions and increase vitality.</p>
            
            <h4 className="font-semibold mb-2">Instructions:</h4>
            <ol className="list-decimal list-inside space-y-1 mb-3">
              <li>Sit comfortably with spine straight</li>
              <li>Take a deep inhale to prepare</li>
              <li>Begin short, sharp exhales through nose by contracting belly</li>
              <li>Let inhales happen naturally between exhales</li>
              <li>Start with 20 pumps, rest, repeat 2-3 rounds</li>
              <li>End with several natural breaths</li>
            </ol>
            
            <p className="text-sm italic">Best for: Depression, lethargy, stuck emotions, mental fog</p>
            <p className="text-sm text-red-600 mt-2"><strong>Caution:</strong> Avoid if pregnant, with heart conditions, or high blood pressure</p>
          </div>

          <div className="bg-green-50 p-6 rounded-lg">
            <h3 className="text-xl font-semibold text-[#bd6334] mb-3">4. Box Breathing for Stability</h3>
            <p className="mb-3">Creates emotional stability and mental clarity through rhythmic breathing.</p>
            
            <h4 className="font-semibold mb-2">Instructions:</h4>
            <ol className="list-decimal list-inside space-y-1 mb-3">
              <li>Sit comfortably with feet flat on floor</li>
              <li>Exhale completely</li>
              <li>Inhale for 4 counts</li>
              <li>Hold for 4 counts</li>
              <li>Exhale for 4 counts</li>
              <li>Hold empty for 4 counts</li>
              <li>Repeat for 5-10 rounds</li>
            </ol>
            
            <p className="text-sm italic">Best for: Anxiety, scattered thoughts, emotional reactivity, focus issues</p>
          </div>

          <div className="bg-pink-50 p-6 rounded-lg">
            <h3 className="text-xl font-semibold text-[#bd6334] mb-3">5. Emotional Release Breathing</h3>
            <p className="mb-3">A more intensive practice for processing and releasing deeper emotional material.</p>
            
            <h4 className="font-semibold mb-2">Instructions:</h4>
            <ol className="list-decimal list-inside space-y-1 mb-3">
              <li>Lie down comfortably in a safe space</li>
              <li>Begin with connected breathing (no pause between inhale/exhale)</li>
              <li>Breathe more deeply and quickly than normal</li>
              <li>Focus on breathing into any areas of tension or emotion</li>
              <li>Allow whatever emotions arise without judgment</li>
              <li>Continue for 10-20 minutes</li>
              <li>Rest and integrate for 5-10 minutes afterward</li>
            </ol>
            
            <p className="text-sm italic">Best for: Deep emotional processing, trauma release, clearing old patterns</p>
            <p className="text-sm text-red-600 mt-2"><strong>Important:</strong> Practice with guidance initially, especially for trauma survivors</p>
          </div>
        </div>

        <h2 className="text-2xl font-semibold text-[#bd6334] mt-8 mb-4">Safety Guidelines for Breathwork</h2>
        <p className="text-lg leading-relaxed mb-6">
          While breathwork is generally safe, certain considerations are important, especially for trauma survivors:
        </p>

        <div className="bg-red-50 p-6 rounded-lg mb-6">
          <h3 className="text-xl font-semibold text-[#bd6334] mb-3">Important Safety Considerations:</h3>
          <ul className="list-disc list-inside space-y-2">
            <li><strong>Start slowly:</strong> Begin with gentle techniques before trying intensive practices</li>
            <li><strong>Stop if overwhelmed:</strong> Return to natural breathing if you feel dizzy or panicked</li>
            <li><strong>Have support available:</strong> Practice intensive breathwork with a trained facilitator initially</li>
            <li><strong>Avoid forcing:</strong> Never strain or push beyond your comfort level</li>
            <li><strong>Medical conditions:</strong> Consult healthcare providers if you have heart, lung, or blood pressure issues</li>
            <li><strong>Pregnancy:</strong> Avoid intensive breathing practices during pregnancy</li>
            <li><strong>Integration time:</strong> Allow time to rest and process after breathwork sessions</li>
          </ul>
        </div>

        <h2 className="text-2xl font-semibold text-[#bd6334] mt-8 mb-4">Creating Your Breathwork Practice</h2>
        <p className="text-lg leading-relaxed mb-6">
          Building a consistent breathwork practice can transform your relationship with emotions and stress:
        </p>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
          <div className="bg-white border border-gray-200 p-5 rounded-lg">
            <h4 className="font-medium mb-3 text-[#bd6334]">Daily Maintenance Practice</h4>
            <ul className="space-y-2 pl-6 list-disc text-sm">
              <li>5-10 minutes of three-part breathing upon waking</li>
              <li>Box breathing during stressful moments</li>
              <li>4-7-8 breathing before sleep</li>
              <li>Brief breath awareness throughout the day</li>
            </ul>
          </div>
          
          <div className="bg-white border border-gray-200 p-5 rounded-lg">
            <h4 className="font-medium mb-3 text-[#bd6334]">Weekly Deeper Practice</h4>
            <ul className="space-y-2 pl-6 list-disc text-sm">
              <li>20-30 minute session with chosen technique</li>
              <li>Focus on one specific emotional theme</li>
              <li>Journal about insights and releases</li>
              <li>Practice in consistent space and time</li>
            </ul>
          </div>
          
          <div className="bg-white border border-gray-200 p-5 rounded-lg">
            <h4 className="font-medium mb-3 text-[#bd6334]">Emergency Emotional Tools</h4>
            <ul className="space-y-2 pl-6 list-disc text-sm">
              <li>Quick 4-7-8 sequence for panic</li>
              <li>Box breathing for overwhelm</li>
              <li>Three-part breath for grounding</li>
              <li>Natural breath observation for presence</li>
            </ul>
          </div>
          
          <div className="bg-white border border-gray-200 p-5 rounded-lg">
            <h4 className="font-medium mb-3 text-[#bd6334]">Monthly Intensive</h4>
            <ul className="space-y-2 pl-6 list-disc text-sm">
              <li>Longer emotional release session</li>
              <li>Work with facilitator if needed</li>
              <li>Focus on releasing old patterns</li>
              <li>Plan integration and self-care afterward</li>
            </ul>
          </div>
        </div>

        <h2 className="text-2xl font-semibold text-[#bd6334] mt-8 mb-4">Working with Emotional Releases</h2>
        <p className="text-lg leading-relaxed mb-6">
          Breathwork can bring up emotions. Here's how to work skillfully with whatever arises:
        </p>

        <div className="bg-yellow-50 p-6 rounded-lg mb-6">
          <h3 className="text-xl font-semibold text-[#bd6334] mb-3">When Emotions Arise During Breathwork:</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <h4 className="font-semibold mb-2">If You Feel Overwhelmed:</h4>
              <ul className="list-disc list-inside space-y-1 text-sm">
                <li>Return to natural breathing immediately</li>
                <li>Open your eyes and ground yourself</li>
                <li>Feel your body's contact with the floor/chair</li>
                <li>Remind yourself you are safe in the present</li>
                <li>Seek support if needed</li>
              </ul>
            </div>
            
            <div>
              <h4 className="font-semibold mb-2">If Emotions Flow Naturally:</h4>
              <ul className="list-disc list-inside space-y-1 text-sm">
                <li>Allow the emotions to move through you</li>
                <li>Breathe into the sensations</li>
                <li>Avoid analyzing or stopping the process</li>
                <li>Trust your body's wisdom</li>
                <li>Continue breathing with whatever arises</li>
              </ul>
            </div>
          </div>
        </div>

        <h2 className="text-2xl font-semibold text-[#bd6334] mt-8 mb-4">Breathwork for Specific Emotions</h2>
        <p className="text-lg leading-relaxed mb-6">
          Different emotions may benefit from specific breathing approaches:
        </p>

        <div className="overflow-hidden rounded-lg border border-gray-200 mb-6">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-[#f8f0e6]">
              <tr>
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">Emotion</th>
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">Recommended Technique</th>
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">Focus</th>
              </tr>
            </thead>
            <tbody className="bg-white divide-y divide-gray-200">
              <tr>
                <td className="px-6 py-4 whitespace-nowrap">
                  <p className="font-medium">Anxiety</p>
                </td>
                <td className="px-6 py-4">
                  <p className="text-sm">4-7-8 Breathing or Box Breathing</p>
                </td>
                <td className="px-6 py-4">
                  <p className="text-sm">Slow, controlled exhalation to activate calm response</p>
                </td>
              </tr>
              <tr className="bg-gray-50">
                <td className="px-6 py-4 whitespace-nowrap">
                  <p className="font-medium">Anger</p>
                </td>
                <td className="px-6 py-4">
                  <p className="text-sm">Cleansing Breath followed by Three-Part Breath</p>
                </td>
                <td className="px-6 py-4">
                  <p className="text-sm">Express energy through exhalation, then soothe with deep breathing</p>
                </td>
              </tr>
              <tr>
                <td className="px-6 py-4 whitespace-nowrap">
                  <p className="font-medium">Sadness/Grief</p>
                </td>
                <td className="px-6 py-4">
                  <p className="text-sm">Emotional Release Breathing</p>
                </td>
                <td className="px-6 py-4">
                  <p className="text-sm">Allow tears and emotions to flow with connected breathing</p>
                </td>
              </tr>
              <tr className="bg-gray-50">
                <td className="px-6 py-4 whitespace-nowrap">
                  <p className="font-medium">Depression</p>
                </td>
                <td className="px-6 py-4">
                  <p className="text-sm">Cleansing Breath</p>
                </td>
                <td className="px-6 py-4">
                  <p className="text-sm">Energize and move stagnant energy through active breathing</p>
                </td>
              </tr>
              <tr>
                <td className="px-6 py-4 whitespace-nowrap">
                  <p className="font-medium">Fear</p>
                </td>
                <td className="px-6 py-4">
                  <p className="text-sm">Three-Part Breath</p>
                </td>
                <td className="px-6 py-4">
                  <p className="text-sm">Ground and stabilize through slow, deep belly breathing</p>
                </td>
              </tr>
            </tbody>
          </table>
        </div>

        <h2 className="text-2xl font-semibold text-[#bd6334] mt-8 mb-4">Integration and Aftercare</h2>
        <p className="text-lg leading-relaxed mb-6">
          What you do after breathwork is as important as the practice itself:
        </p>

        <div className="bg-green-50 p-6 rounded-lg mb-6">
          <h3 className="text-xl font-semibold text-[#bd6334] mb-3">Post-Breathwork Integration:</h3>
          <ul className="list-disc list-inside space-y-2">
            <li><strong>Rest quietly:</strong> Allow 5-10 minutes of stillness after practice</li>
            <li><strong>Stay hydrated:</strong> Drink water to support the body's natural detox process</li>
            <li><strong>Journal insights:</strong> Write down any emotions, memories, or realizations that arose</li>
            <li><strong>Gentle movement:</strong> Light stretching or walking can help integrate the experience</li>
            <li><strong>Avoid overstimulation:</strong> Choose calm activities for several hours afterward</li>
            <li><strong>Practice self-compassion:</strong> Honor whatever came up without judgment</li>
          </ul>
        </div>

        <div className="bg-blue-50 p-6 rounded-lg">
          <h3 className="text-xl font-semibold text-[#bd6334] mb-3">Today's Practice:</h3>
          <p className="text-lg leading-relaxed mb-4">
            Choose one breathwork technique from this lesson and practice it for 10-15 minutes. Start with three-part breathing if you're new to breathwork, or select the technique that most appeals to your current emotional state.
          </p>
          <p className="text-lg leading-relaxed italic">
            "With each conscious breath, you reclaim your power to transform and heal. Let your breath be your guide back to wholeness."
          </p>
        </div>
      </div>
    </div>
  )
}

export default Module5Lesson6